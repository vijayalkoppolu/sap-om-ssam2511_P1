...
export default function DeleteUnusedOverviewEntities(context) {
    ...
    queryBuilder.addFilter('sap.hasPendingChanges()');
...
function executeDeleteChain(context, entities) {
    ...
    return context.count('/SAPAssetManager/Services/AssetManager.service', 'Confirmations', [], `$filter=sap.hasPendingChanges() and PostingDate eq datetime'${date}'`).then(count => {
        if (count > 0) {
