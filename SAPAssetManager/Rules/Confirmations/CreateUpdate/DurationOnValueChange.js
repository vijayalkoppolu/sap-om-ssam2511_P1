...
export default function DurationOnValueChange(formCellControlProxy) {
    ...
    if (endDateTime > now) { //Only adjust the start date and time if the new duration causes the end date and time to be in the future
        Logger.info('FCTCLog', 'FCTCLog - TIMESHEET <DurationOnValueChange> endDateTime is greater than current datetime' );
        ...
        Logger.info('FCTCLog', 'FCTCLog - TIMESHEET <DurationOnValueChange> StartDateTime ' + startDateControl.getValue());
        ...
        if (getPostingOverride(formCellControlProxy)) {
            ...
            Logger.info('FCTCLog', 'FCTCLog - TIMESHEET <DurationOnValueChange> PostingDateTime ' + postingDateControl.getValue());

        }

        let startTimeControl = libCom.getControlProxy(pageProxy, 'StartTimePicker');
        startTimeControl.setValue(startDateTime.date());
        Logger.info('FCTCLog', 'FCTCLog - TIMESHEET <DurationOnValueChange> Start time ' + startTimeControl.getValue());
    }
}
