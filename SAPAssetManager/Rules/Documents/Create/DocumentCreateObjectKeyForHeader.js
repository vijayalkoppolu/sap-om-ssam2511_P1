...
export default function DocumentCreateObjectKeyForHeader(controlProxy, bindingObject = controlProxy.binding) {
    const binding = CommonLib.getPageName(controlProxy) === 'PDFControl'
        ? CommonLib.getStateVariable(controlProxy, 'ServiceReportData')
        : bindingObject;
    let value = binding['@odata.readLink'];
    switch (DocLib.getParentObjectType(controlProxy, binding['@odata.type'])) {
