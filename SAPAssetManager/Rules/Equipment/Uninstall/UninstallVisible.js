...
export default function UninstallVisible(context) {
    let query = context.binding['@odata.type'] === '#sap_mobile.MyFunctionalLocation' ?
        `$expand=SystemStatuses_Nav&$filter=sap.entityexists(SystemStatuses_Nav) and FunctionalLocation/FuncLocId eq '${context.binding.FuncLocId}' and SuperiorEquip eq ''` :
