...
export default function WorkOrderReAssignChangeSet(context) {
    if (context.binding &&  context.binding['@odata.type'] === '#sap_mobile.MyWorkOrderHeader') {
        ...
        //unassign && assign
        return context.read('/SAPAssetManager/Services/AssetManager.service', 'MyWorkOrderPartners', [] ,`$filter=PartnerFunction eq 'VW' and PersonNum eq '${context.getClientData().PreviousEmployeeTo}' and OrderId eq '${context.binding.OrderId}' and sap.hasPendingChanges()`).then(function(results) {
            if (results && results.length >0) {
                ...
                return context.executeAction('/SAPAssetManager/Actions/Supervisor/Assign/WorkOrderAssignPartnerDelete.action').then(() =>{
                    return context.read('/SAPAssetManager/Services/AssetManager.service', 'WorkOrderTransfers', [], `$filter=OrderId eq '${context.binding.OrderId}' and EmployeeTo eq '${context.getClientData().PreviousEmployeeTo}' and sap.hasPendingChanges()`).then(function(transferResults) {
