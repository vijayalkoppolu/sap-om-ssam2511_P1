...
export default function OperationReAssignChangeSet(context) {
    if (context.binding &&  context.binding['@odata.type'] === '#sap_mobile.MyWorkOrderOperation') {
        ...
        if (!(libVal.evalIsEmpty(context.binding.Employee_Nav)) || context.binding.PersonNum !== '00000000') {
            ...
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'WorkOrderTransfers', [], `$filter=OrderId eq '${context.binding.OrderId}' and OperationNo eq '${context.binding.OperationNo}' and EmployeeTo eq '${context.getClientData().PreviousEmployeeTo}' and sap.hasPendingChanges()`).then(function(results) {
