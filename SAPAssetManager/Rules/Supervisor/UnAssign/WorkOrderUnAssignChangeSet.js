...
export default function WorkOrderUnAssignChangeSet(context) {
    if (context.binding &&  context.binding['@odata.type'] === '#sap_mobile.MyWorkOrderHeader') {
        ...
        return context.read('/SAPAssetManager/Services/AssetManager.service', 'MyWorkOrderPartners', [] ,`$filter=PartnerFunction eq 'VW' and PersonNum eq '${employeeTo}' and OrderId eq '${orderId}' and sap.hasPendingChanges()`).then(function(results) {
            if (results && results.length >0) {
                ...
                return context.executeAction('/SAPAssetManager/Actions/Supervisor/Assign/WorkOrderAssignPartnerDelete.action').then(() =>{
                    return context.read('/SAPAssetManager/Services/AssetManager.service', 'WorkOrderTransfers', [], `$filter=OrderId eq '${context.binding.OrderId}' and EmployeeTo eq '${context.getClientData().EmployeeTo}' and sap.hasPendingChanges()`).then(function(transferResults) {
