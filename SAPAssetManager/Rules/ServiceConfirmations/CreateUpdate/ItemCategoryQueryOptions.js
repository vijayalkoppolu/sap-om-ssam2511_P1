...
export default function ItemCategoryQueryOptions(context) {
    ...
    return readType.then(type => {
        ...
        if (pageName === 'CreateUpdateServiceItemScreen' || pageName === 'CreateUpdateServiceQuotationItem') {
            ...
            if (previousPageName === 'StockListViewPage' || previousPageName === 'StockDetailPage') {
                categories = S4ServiceLibrary.getServiceProductPartCategories(context);

                if (context.binding?.MaterialNum && !productId) {
                    productId = context.binding.MaterialNum;
                }
            }
