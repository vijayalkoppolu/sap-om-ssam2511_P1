...
function partialSearchODataResult(odataResult, context) {
    ...
    let filterOptions = [];
    let readLinkValue = odataResult.OdataBinding['@odata.readLink'];
    let entityType = odataResult.OdataBinding['@odata.type'].substring('#sap_mobile.'.length);
    // Extract the entity set name
    const entitySetName = readLinkValue.substring(0, readLinkValue.indexOf('('));

    // Extract the key-value pairs
    const keyValueString = readLinkValue.substring(readLinkValue.indexOf('(') + 1, readLinkValue.lastIndexOf(')'));
    const keyValuePairs = keyValueString.split(',');

    // Convert key-value pairs to query options
    let queryOptions = keyValuePairs.map(pair => {
        const [key, value] = pair.split('=');
        return `${key} eq ${decodeURIComponent(value)}`;
    }).join(' and ');
    if (queryOptions) {
...
function getSearchQuery(context, searchString, entityType) {
    let searchQuery = '';
    if (searchString) {
        let searchByProperties = ['Material', 'ReferenceDocNumber', 'HandlingDecision', 'DestStorLocID', 'FldLogsHandlingDecision_Nav/Description'];
