    ...
    function validateOffsets(context, dict) {
        ...
        if (libCom.isDefined(dict[Offset1TypeLstPkr].getValue()) && libCom.isDefined(dict[Offset2TypeLstPkr].getValue())) {
            ...
            if (dict[Offset1TypeLstPkr].getValue()[0].ReturnValue === dict[Offset2TypeLstPkr].getValue()[0].ReturnValue) {
                ...
                /// Offsets has to be either numeric or empty string
               let offset1Valid = dict[Offset1].getValue() === '' || libLocal.isNumber(context, dict[Offset1].getValue());
                let offset2Valid = dict[Offset2].getValue() === '' || libLocal.isNumber(context, dict[Offset2].getValue());
                if (offset1Valid && offset2Valid) {
                    return Promise.resolve(true);
                } else {
