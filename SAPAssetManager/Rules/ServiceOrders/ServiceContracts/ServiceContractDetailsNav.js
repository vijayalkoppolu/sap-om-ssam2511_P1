...
export default function ServiceContractDetailsNav(context) {
    ...
    return context.read('/SAPAssetManager/Services/AssetManager.service', binding['@odata.readLink'] + '/TransHistories_Nav', [], `$filter=sap.entityexists(${filter})&${expand}`).then(results => {
        if (results.length > 0) {
            if (type === '#sap_mobile.S4ServiceItem') {
