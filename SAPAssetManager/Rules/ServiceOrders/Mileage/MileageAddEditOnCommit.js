    ...
    try {
        if (IsOnCreate(pageProxy)) {
            ...
            return GenerateLocalConfirmationNum(pageProxy).then(confirmationNum => { //Generate a new confirmation number and store it
                ...
                return TelemetryLibrary.executeActionWithLogUserEvent(pageProxy,
                    '/SAPAssetManager/Actions/ServiceOrders/Mileage/MileageAdd.action',
                    pageProxy.getGlobalDefinition('/SAPAssetManager/Globals/Features/MileageReport.global').getValue(),
                    TelemetryLibrary.EVENT_TYPE_CREATE).then((result) => { //Create the Mileage Confirmation
