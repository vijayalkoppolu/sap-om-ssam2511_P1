...
import WorkOrderDeleteGeometryPost from './WorkOrderDeleteGeometryPost';
    ...
    if (context.binding.isServiceOrder) {
        ...
        return updateSOPartner(context, currentPartnerReadLink, currentPartnerID, newPartnerID).then(() => {
            return updateAccountingIndicator(context).then(() => {
                return createUpdateDeleteGeometry(context).then(() => {
...
function createUpdateDeleteGeometry(context) {
    ...
    if (context.getClientData().GeometrySubmitDeletion) {
        return WorkOrderDeleteGeometryPost(context);
    }
...
function updateSOPartner(context, currentPartnerReadLink, currentPartnerID, newPartnerID) {
    ...
    //delete and recreate WOPartner when editing a local order
    let isLocal = libCommon.isEntityLocal(context.binding);
    if (isLocal) {
        return context.executeAction('/SAPAssetManager/Actions/ServiceOrders/ServiceOrderPartnerDelete.action').then(() => {
            return context.executeAction('/SAPAssetManager/Actions/ServiceOrders/ServiceOrderPartnerCreateRelated.action');
        });
    }
