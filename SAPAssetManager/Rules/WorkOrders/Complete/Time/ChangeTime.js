...
export default function ChangeTime(context) {
    ...
    } else if (WorkOrderCompletionLibrary.getInstance().isOperationSplitFlow() && binding.PMMobileStatus_Nav) {
        mobileStatus = binding.PMMobileStatus_Nav.MobileStatus;
    }
...
function openConfirmationPage(context, binding, isReviewRequired) {
    ...
    if (WorkOrderCompletionLibrary.getInstance().isWOFlow()) {
        ...
        overrides = {
            ...
            'WorkOrderHeader': operation.WOHeader,
            'Operation': operation.OperationNo,
            'MobileStatus': binding.MobileStatus,
            'IsFinalChangable': false,
            'Plant' : operation.MainWorkCenterPlant,
            'doCheckOperationComplete' : false,
            'OperationActivityType': operation.ActivityType,
            'IsFinal': true,
            'FinalConfirmation': 'X',
        };
