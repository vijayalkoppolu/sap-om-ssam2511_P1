...
export default async function WorkOrderNotificationCreateNav(context) {
    ...
    await notifCreateChangeSetNav(context, bindingObject);
    ...
    if (localId) {
        ...
        let notif = await context.read('/SAPAssetManager/Services/AssetManager.service', "MyNotificationHeaders('" + localId + "')", ['NotificationNumber'], '');
        if (notif && notif.length > 0) {
            ...
            if (!binding.NotificationNumber) { //Link this new notification to the WO header since there isn't one already linked
                binding.NotificationNumber = localId;

                const sectionedTableControl = context?.getPageProxy()?.getControl('SectionedTable');
                const notificationsList = sectionedTableControl?.getSection('NotificationsList');

                if (notificationsList) {
                    await notificationsList?.redraw(); //Force the notification section to redraw for Android
                }
