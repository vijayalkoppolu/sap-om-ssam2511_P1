...
export default function HideCancelOnPageLoad(context) {
    ...
    helper.run((sectionBinding, section) => {
        ...
        return context.read('/SAPAssetManager/Services/AssetManager.service', readPath, [], `$filter=sap.hasPendingChanges() and Register eq '${sectionBinding.RegisterNum}'&$orderby=MeterReadingDate`).then(function(result) {
            if (result.length > 0) {
