    ...
    sectionHelper.run((sectionBinding, section) => {
        ...
        if (validationResult.every(validation => validation === true)) {
            return sectionHelper.run((sectionBinding, section, sectionIndex) => {
                ...
                return context.read('/SAPAssetManager/Services/AssetManager.service', 'MeterReadings', [], `$filter=sap.hasPendingChanges() and RegisterGroup eq '${sectionBinding.RegisterGroup}' and Register eq '${sectionBinding.RegisterNum}' and EquipmentNum eq '${equipmentNum}'`).then(function(result) {
                    let readLink = '';
