...
export default class {
    ...
    static crewListItemProcessVehicleLoop(context) {
        ...
        if (counter === vehicles.length) { //We are done processing rows
            ...
            if (Object.prototype.hasOwnProperty.call(dict,rowKey)) {
                ...
                context.getPageProxy().getClientData().CrewListItemReadLink = row['@odata.readLink'];
    ...
    static initializeCrewHeader(context) {
        ...
        try {
            return context.read('/SAPAssetManager/Services/AssetManager.service','CrewLists',[],`$top=1&$filter=SAPUserName eq '${libCom.getSapUserName(context)}' and OriginTimeStamp eq ${odataDate} and ${FSMCrewLibrary.getQueryWithExcludedFSMCrew(context)}`).then(result => {
                if (result && result.length > 0) {
    ...
    static initializeCrewHeaderAndEmployee(context, date, employee) {
        ...
        try {
            return context.read('/SAPAssetManager/Services/AssetManager.service', 'CrewLists', [], `$top=1&$filter=SAPUserName eq '${sapUserName}' and OriginTimeStamp eq ${odataDate} and ${FSMCrewLibrary.getQueryWithExcludedFSMCrew(context)}`).then(result => {
    ...
    */
    ...
    static getTotalHoursWithDateForCurrentCrew(context, date) {
        ...
        const sapUserName = libCom.getSapUserName(context);
        const excludedFSMCrewQuery = FSMCrewLibrary.getQueryWithExcludedFSMCrew(context, true);
        const queryOptions = `$expand=Employee&$orderby=Employee/LastName&$filter=(RemovalFlag ne 'X' or sap.hasPendingChanges()) and CrewList/SAPUserName eq '${sapUserName}' and CrewList/OriginTimeStamp eq ${odataDate} and CrewItemType eq 'EMPLOYEE' and ${excludedFSMCrewQuery}`;
    ...
    static getTotalCrewMembersWithDate(context, date) {
        ...
        const odataDate = "datetime'" + new ODataDate(date).toLocalDateString() + "'";
        const queryOpts = `$filter=RemovalFlag ne 'X' and CrewList/SAPUserName eq '${libCom.getSapUserName(context)}' and CrewList/OriginTimeStamp eq ${odataDate} and CrewItemType eq 'EMPLOYEE' and ${FSMCrewLibrary.getQueryWithExcludedFSMCrew(context, true)}`;
