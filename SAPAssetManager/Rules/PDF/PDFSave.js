    ...
    return pdfPath(clientAPI).then((path) => {
        ...
        if (!readError) {
            ...
            return libTelemetry.executeActionWithLogUserEvent(clientAPI,
                '/SAPAssetManager/Actions/Documents/DocumentCreateBDS.action',
                clientAPI.getGlobalDefinition('/SAPAssetManager/Globals/Features/ServiceReport.global').getValue(),
                libTelemetry.EVENT_TYPE_CREATE).then((data) => {
