...
import PDFTemplatePath from './PDFTemplatePath';

export default async function PDFGenerate(context, completionBinding) {
    const templateExists = await checkIfReportTemplateExists(context);
    if (templateExists) {
        ...
        if (binding['@odata.type'] === '#sap_mobile.S4ServiceOrder' || binding['@odata.type'] === '#sap_mobile.S4ServiceItem') {
            const info = await prepareS4OrderInfo(context, binding);
            return setUpBindingAndOpenPDF(context, binding, info);
        }

        const info = await prepareOrderInfo(context, binding);
        return setUpBindingAndOpenPDF(context, binding, info);
    }
    ...
    return context.executeAction({
        ...
        'Properties': {
            'Title': context.localizeText('failed_render_service_report'),
            'Message': context.localizeText('no_service_report_templates_available'),
...
async function checkIfReportTemplateExists(context) {
    try {
        const templatePath = await PDFTemplatePath(context, false);
        return common.isDefined(templatePath);
    } catch (error) {
        Logger.error(context.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryDocuments.global').getValue(), 'Error getting template for pdf: ' + error);
        return false;
    }
}
