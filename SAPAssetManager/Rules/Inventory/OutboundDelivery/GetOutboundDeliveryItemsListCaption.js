...
import GetOutboundDeliveryListQuery from './GetOutboundDeliveryItemsListQuery';
import FetchRequest from '../../Common/Query/FetchRequest';

export default async function GetOutboundDeliveryItemsListCaption(clientAPI) {
    ...
    const totalCountPromise = ComLib.getEntitySetCount(clientAPI, 'OutboundDeliveryItems', queryOptions);
...
async function getCountPropertyValue(clientAPI) {
    const queryBuilder = await GetOutboundDeliveryListQuery(clientAPI, false, false);
    const items = await queryBuilder.build().then(query => {
        const fetchRequest = new FetchRequest('OutboundDeliveryItems', query);
