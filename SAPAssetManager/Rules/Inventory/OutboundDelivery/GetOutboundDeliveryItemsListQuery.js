...
export default function GetOutboundDeliveryListQuery(context, queryOnly = false, caption = true) {
    ...
    let baseQueryInitial = baseQuery; // Store initial base query for filters
    ...
    let tabGroups;
    if (tabFilters) {
        tabGroups = tabFilters.flatMap(val => val.filterItems);
...
export function getFilterQuery(context, baseQueryInitial) {
  const filterOptions = [];
