...
export default function GetOutboundListQuery(context, queryOnly=false, withSearch=false) {
    ...
    return context.read('/SAPAssetManager/Services/AssetManager.service', 'OnDemandObjects', [], '$orderby=ObjectId').then(results => {
        ...
        if (searchString) { //Supporting order number, material number and EanUpc for searches
            let idSearchString = '';
            ...
            filters.push(`PurchaseOrderHeader_Nav/PurchaseOrderItem_Nav/any(wp : substringof('${searchString}', tolower(wp/Material_Nav/EanUpc)))`);
            ...
            filters.push(`StockTransportOrderHeader_Nav/StockTransportOrderItem_Nav/any(wp : substringof('${searchString}', tolower(wp/Material_Nav/EanUpc)))`);
            filters.push(`InboundDelivery_Nav/Items_Nav/any(wp : substringof('${searchString}', tolower(wp/Material)))`);
            filters.push(`InboundDelivery_Nav/Items_Nav/any(wp : substringof('${searchString}', tolower(wp/Material_Nav/EanUpc)))`);
            filters.push(`OutboundDelivery_Nav/Items_Nav/any(wp : substringof('${searchString}', tolower(wp/Material)))`);
            filters.push(`OutboundDelivery_Nav/Items_Nav/any(wp : substringof('${searchString}', tolower(wp/Material_Nav/EanUpc)))`);
        }
        if (filters.length > 0) {
            if (baseQuery) {
