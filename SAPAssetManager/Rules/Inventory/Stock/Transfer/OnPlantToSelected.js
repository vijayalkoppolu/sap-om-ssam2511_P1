...
export default function OnPlantToSelected(context) {
    ...
    if (context.getValue().length > 0) {
        ...
        const materialNum = context?.binding?.MaterialNum || context.getPageProxy().getClientData().Material;
        ...
        if (materialNum && value) {
            ...
            batchToSpec.setDisplayValue('{Batch}');
            batchToSpec.setReturnValue('{Batch}');
