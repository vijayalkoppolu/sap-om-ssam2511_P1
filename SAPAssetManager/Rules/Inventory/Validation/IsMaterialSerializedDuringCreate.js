export default function IsMaterialSerializedDuringCreate(context) {
    ...
    try {
        ...
        let plant;
        /*Split the  material and plant value from the IssueOrReceiptCreateUpdate page */
        const regex = /MaterialPlants\(Plant='([^']+)',MaterialNum='([^']+)'\)/;
        const match = material.match(regex);
        if (match) {
            plant = match[1];
            material = match[2];
        } else if (context.getPageProxy().getControl('FormCellContainer').getControl('PlantLstPkr')) {
