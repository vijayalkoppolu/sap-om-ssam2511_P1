...
export default function SerialNumbersTarget(context) {
    ...
    if (!serialNumbers) {
        ...
        if (type === 'MaterialDocItem') {
            let headArr = [];
            arr = GetMaterialNumMaterialDocItemForBulkUpdate(target, arr, movementType === 'R', !!libCom.getStateVariable(context, 'MaterialDocumentBulkUpdate'));
...
function CreateArrForMaterialDocItem(target, headArr, arr) {
    if (headArr.length) {
        ...
        for (let arrItem of arr) {
            const copyItem = headArr.find(item => item.SerialNumber === arrItem.SerialNumber);
            if (copyItem) {
                arrItem.copy = true;
...
function GetMaterialNumMaterialDocItemForBulkUpdate(target, arr, isReceipt = false, isBulkUpdate = false) {
    if (isBulkUpdate && (target.PurchaseOrderItem_Nav || target.StockTransportOrderItem_Nav)) {
        const isSTO = !!(target.StockTransportOrderItem_Nav);
        const newTarget = target.PurchaseOrderItem_Nav || target.StockTransportOrderItem_Nav;
        return GetMaterialNumMaterialDocItem(newTarget, arr, isSTO, isReceipt);
    }
    return arr;
}
