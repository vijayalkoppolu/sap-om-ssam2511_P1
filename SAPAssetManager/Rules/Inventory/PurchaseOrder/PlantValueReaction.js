...
export default function PlantValueReaction(context) {
    ...
    if (context.getValue().length > 0) {
        ...
        if (plant) {
            ...
            slocEditable = true;
            if ([MovementTypes.t301, MovementTypes.t303].includes(type)) {
                plantQuery = `$filter=Plant ne '${plant}'&$orderby=Plant`;
                plantEditable = true;
            } else if ([MovementTypes.t305, MovementTypes.t311, MovementTypes.t313, MovementTypes.t315, MovementTypes.t321, MovementTypes.t322, MovementTypes.t343, MovementTypes.t411].some(t => t === type)) {
