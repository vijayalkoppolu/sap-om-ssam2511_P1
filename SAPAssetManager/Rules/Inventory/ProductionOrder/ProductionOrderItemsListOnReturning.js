...
export default function ProductionOrderItemsListOnReturning(context) {
    ...
    const section = sectionedTableProxy.getSection('PRDItemsSectionObjectTable');
    if (section) {
        section.redraw(true);
    }

    sectionedTableProxy.redraw().then(()=>{
        getCaptionState(context, 'ProductionOrderItemsListPage');
    });
}
