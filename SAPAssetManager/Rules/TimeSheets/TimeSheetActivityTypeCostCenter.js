...
export default function  TimeSheetActivityTypeCostCenter(context) {
    ...
    let workOrder = '';
    try {
        workOrder = libCom.getListPickerValue(libCom.getTargetPathValue(context, '#Control:RecOrderLstPkr/#Value'));
    } catch (error) {
        console.error('Error getting work order value:', error);
        // workOrder remains an empty string
    }
    if (workOrder) {
