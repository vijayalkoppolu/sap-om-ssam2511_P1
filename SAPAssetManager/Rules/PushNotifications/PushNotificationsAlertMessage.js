...
export default function PushNotificationsAlertMessage(context, appEventData) {
    ...
    const pushNotificationHeader = getHeaderKey(context, unifiedPayload[getUnifiedKey(context, 'localizedTitle')]);
...
function getHeaderKey(context, key) {
    if (key === 'WO_TITLE_KEY' && PersonaLibrary.isFieldServiceTechnician(context))
        return 'SO_TITLE_KEY';
    return key;
}
