    ...
    if (userFeaturesLib.isFeatureEnabled(context,context.getGlobalDefinition('/SAPAssetManager/Globals/Features/QM.global').getValue())) {
        ...
        switch (context.getProperty()) {
            case 'Title':
                return libForm.getFormattedKeyDescriptionPair(context, binding.ItemNumber, binding.ItemText);
            case 'Subhead':
                return readPartCode(context, binding).then(function(data) {
                    if (data && data.length > 0) {
                        return data.getItem(0).CodeDescription;
                    } else {
                        return '';
            ...
            case 'Footnote':
                return readDamageCode(context, binding).then(function(codeData) {
                    if (codeData && codeData.length > 0) {
                        return libForm.getFormattedKeyDescriptionPair(context, binding.DamageCode, codeData.getItem(0).CodeDescription);
                    } else {
                        return context.localizeText('no_damage_code_specified');
...
export function readPartCode(context, bindingObj) {
    const binding = bindingObj || context.binding;
    return context.read('/SAPAssetManager/Services/AssetManager.service', `PMCatalogCodes(Catalog='B', Code='${binding.ObjectPart}', CodeGroup='${binding.ObjectPartCodeGroup}')`, [], '');
}

export function readDamageCode(context, bindingObj) {
    const binding = bindingObj || context.binding;
    return context.read('/SAPAssetManager/Services/AssetManager.service', `PMCatalogCodes(Catalog='C', Code='${binding.DamageCode}', CodeGroup='${binding.CodeGroup}')`, [], '');
}
