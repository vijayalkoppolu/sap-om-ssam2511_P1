...
export default function NotificationCreateUpdateQMCodeGroupOnValueChange(listPickerProxy) {
    ...
    if (selection.length > 0) {
        ...
        Object.keys(failureGroupObject).forEach(key => failureGroupObject[key] = decodeURIComponent(failureGroupObject[key]));

        let filter = `$filter=Catalog eq '${failureGroupObject.Catalog}' and CodeGroup eq '${failureGroupObject.CodeGroup}'&$orderby=Code`;
