...
export class SubOperationControlLibrary {
    ...
    static getOperationNo(pageProxy) {
        if (pageProxy.getClientData().ParentOperation) {
            ...
            return opNum;
        } else if (pageProxy?.binding?.OperationNo) {
            libCommon.setStateVariable(pageProxy, 'operationId', pageProxy.binding.OperationNo);
            libCommon.setStateVariable(pageProxy, 'workOrderId', pageProxy.binding.OrderId);
            return pageProxy.binding.OperationNo;
        } else {
            let formCellContainer = pageProxy.getControl('FormCellContainer');
