...
import { WH_PHYINV_COUNTED_FILTER } from  '../WHPhysicalInvListQuery';
import Logger from '../../../Log/Logger';
export default function PhyInvCountedFilterDisplayValue(clientAPI) {
    let queryOptions = `$filter=(${WH_PHYINV_COUNTED_FILTER})`;
    ...
    return clientAPI.count('/SAPAssetManager/Services/AssetManager.service', 'WarehousePhysicalInventoryItems', queryOptions).then(count => {
        return clientAPI.localizeText('counted_x', [count]);
    })
    .catch(error => {
