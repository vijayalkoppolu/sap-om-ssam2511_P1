...
import InspectionCharacteristicsUpdateQueryOptions from '../../../SAPAssetManager/Rules/InspectionCharacteristics/Update/InspectionCharacteristicsUpdateQueryOptions';
import InspectionCharacteristicsUpdateEntitySet from '../../../SAPAssetManager/Rules/InspectionCharacteristics/Update/InspectionCharacteristicsUpdateEntitySet';
/**
* Describe this function...
* @param {IClientAPI} clientAPI
*/
/**
* ZEquinorSSAM override of InspectionCharacteristicsPageMetadataGenerator
* Adjusts header to show:
* 1) Functional Location (FL ID)
* 2) Equipment
* 3) Checklist ID
* Removes Plant from header body.
*/
let count;
let sectionIndex;
let bindings;
            ...
            if (results && results.length > 0) {
                for (let i = 0; i < results.length; i++) {
                    bindings[i] = results.getItem(i);
                        if (bindings[i].FunctionalLocation) {
...
/**
 * GAP - NGE-102966 : ZEquinorSSAM addHeaderSection
 * Primary identifier: Functional Location (FL ID)
 * Second line: Equipment (if available)
 * Third line: Checklist ID (if available). If not available, falls back to InspectionLot.
 * Plant information removed from header body.
 */
export function addHeaderSection(context, binding, page) {
    let entityset = '';
    let floc = binding.FunctionalLocation || binding.FuncLocId || (binding.InspectionLot_Nav && binding.InspectionLot_Nav.FunctionalLocation) || (binding.EAMChecklist_Nav && binding.EAMChecklist_Nav.FunctionalLocation);
    let equipment = binding.Equipment || binding.EquipId || binding.EquipNum || (binding.InspectionLot_Nav && binding.InspectionLot_Nav.Equipment) || (binding.EAMChecklist_Nav && binding.EAMChecklist_Nav.Equipment);
    let equipdesc = '';
    equipdesc=binding.EquipDesc || (binding.InspectionLot_Nav && binding.InspectionLot_Nav.EquipDesc) || (binding.EAMChecklist_Nav && binding.EAMChecklist_Nav.EquipDesc);
    let headlineText = '-';
    let bodyText = '-';
    //GAP - NGE-102966: Checklist ID as third line; fallback to InspectionLot when not available
    let footnote = binding.ChecklistID || binding.ChecklistId || binding.InspectionLot || '';
    let statusText = '-';
    if (floc) {
    ...
    if (entityset) {
        return read(context, entityset, [], '').then(async function(result) {
            if (result && result.length > 0) {
                ...
                if (result.getItem(0)['@odata.type'] === '#sap_mobile.MyFunctionalLocation') {
                    // GAP - NGE-102966 Headline is functional location with description
                    headlineText = result.getItem(0).FuncLocId + ' - ' + result.getItem(0).FuncLocDesc;
                    // GAP - NGE-102966 Show equipment and equipment description as second line when available
                    if (equipment) {
                        //GAP - NGE-102966 Try to get equipment description from the result if available
                        let equipmentDesc = await read(context, `MyEquipments('${equipment}')`, [], '').then( function(result) {
            if (result && result.length > 0) {
                equipdesc =  result.getItem(0).EquipDesc;
                return equipdesc;
            }
            });
                        if (!equipmentDesc && result.getItem(0).EquipId === equipment) {
                            equipmentDesc = result.getItem(0).EquipDesc;
                        }
                        bodyText = equipment + (equipmentDesc ? ' - ' + equipmentDesc : '');
                    } else {
                        bodyText = '-';
                    }
                } else if (result.getItem(0)['@odata.type'] === '#sap_mobile.MyEquipment') {
                    // Headline is equipment (no FL available)
                    headlineText = result.getItem(0).EquipId + ' - ' + result.getItem(0).EquipDesc;
                    // If FL exists in binding, show it as second line; otherwise leave blank
                    if (floc) {
                        bodyText = floc;
                    } else {
                        bodyText = '-';
            ...
            // compute status counts (same as original)
